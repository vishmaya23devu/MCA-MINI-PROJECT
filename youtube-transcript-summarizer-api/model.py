...
def nlp_model(v_id):
    ...
    if (transcript == '0'):
        ...
        mal_translated_summary = g_translate(english_summary, 'ml')

        hin_translated_summary = g_translate(english_summary, 'hi')
        tam_translated_summary = g_translate(english_summary, 'ta')
        print(tam_translated_summary)
        # print(original_text_length, '-->', final_summary_length)
        # print(final_smy)

        makeTextFile("English", english_summary)
        makeTextFile("Malayalam", mal_translated_summary)
        makeTextFile("Hindi", hin_translated_summary)
        makeTextFile("Tamil", tam_translated_summary)


        return original_text_length, final_summary_length, english_summary, mal_translated_summary, hin_translated_summary,tam_translated_summary
